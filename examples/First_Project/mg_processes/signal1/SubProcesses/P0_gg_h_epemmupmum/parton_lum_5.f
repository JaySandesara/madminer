...
C        
C     
C     Process: g b > h > e+ e- mu+ mu- b NP<=2 QCD<=5 QED<=4
C      WEIGHTED<=28 [ all = QCD ]
C     Process: g c > h > e+ e- mu+ mu- c NP<=2 QCD<=5 QED<=4
C      WEIGHTED<=28 [ all = QCD ]
C     Process: g d > h > e+ e- mu+ mu- d NP<=2 QCD<=5 QED<=4
C      WEIGHTED<=28 [ all = QCD ]
C     Process: g s > h > e+ e- mu+ mu- s NP<=2 QCD<=5 QED<=4
C      WEIGHTED<=28 [ all = QCD ]
C     Process: g u > h > e+ e- mu+ mu- u NP<=2 QCD<=5 QED<=4
C      WEIGHTED<=28 [ all = QCD ]
C     
C     ****************************************************            
C         
...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! g b > e+ e- mu+ mu- b
        PD(IPROC) = G1*B2
        IPROC=IPROC+1  ! g c > e+ e- mu+ mu- c
        PD(IPROC) = G1*C2
        IPROC=IPROC+1  ! g d > e+ e- mu+ mu- d
        PD(IPROC) = G1*D2
        IPROC=IPROC+1  ! g s > e+ e- mu+ mu- s
        PD(IPROC) = G1*S2
        IPROC=IPROC+1  ! g u > e+ e- mu+ mu- u
        PD(IPROC) = G1*U2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
