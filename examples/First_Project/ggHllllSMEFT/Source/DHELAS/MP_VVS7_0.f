...
C     P(1,2)*P(2,1) - (9*P(1,3)*P(2,1))/25. - (9*P(1,2)*P(2,3))/25. -
C      (16*P(-1,1)*P(-1,2)*Metric(1,2))/25. + (9*P(-1,2)**2*Metric(1,2))
C     /25. + (3*P(-1,1)*P(-1,3)*Metric(1,2))/25. + (12*P(-1,2)*P(-1,3)*
C     Metric(1,2))/25. - (6*P(-1,3)**2*Metric(1,2))/25.
C     
      SUBROUTINE MP_VVS7_0(V1, V2, S3, COUP,VERTEX)
      IMPLICIT NONE
      COMPLEX*32 CI
      ...
      VERTEX = COUP*9Q0/25Q0 * S3(5)*(TMP13*(-2Q0/3Q0)*(+3Q0/2Q0 * CI
     $ *(TMP14)-8Q0/3Q0 * CI*(TMP2)-CI*(TMP17)+2Q0 * CI*(TMP16)+1Q0
     $ /2Q0 * CI*(TMP15))+(TMP11*25Q0/9Q0*(-CI*(TMP8)+9Q0/25Q0 * CI
     $ *(TMP9))+CI*(TMP8*TMP12)))
      END
